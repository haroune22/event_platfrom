...
const CommunityEvents = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["events", community.id, page],
    queryFn: () => fetchCommunityEvents(community.id, page),
    retry: false,
  })
  ...
  if (error) {
    return (
      <div className="flex max-w-3xl mx-auto flex-1 flex-col gap-4">
  ...
  const hasNoEvents = !data?.events || data.events.length === 0
      ...
      <div className="mx-auto flex max-w-3xl flex-1 flex-col gap-4">
        ...
        {hasNoEvents ? (
          ...
          <div className="flex max-w-4xl flex-col gap-4">
            {data.events?.map((e: Post) => (
              <PostCard key={e.id} post={e} />
            ))}
          </div>
        )}

        {!hasNoEvents && data.totalPages > 1 && (
